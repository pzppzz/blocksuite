...
export class AIPanelError extends WithDisposable(LitElement) {
  ...
  private _getResponseGroup = () => {
    ...
    if (this.config.error?.type !== AIErrorType.GeneralNetworkError) {
    ...
    responseGroup = this.config.responses
      .map(group => ({
        ...
        items: group.items.filter(item =>
          item.showWhen ? item.showWhen() : true
        ),
      }))
    ...
    const errorTemplate = choose(
      ...
      () => html`
        ...
        <div class="error-info">
          You've reached the current usage cap for AFFiNE AI. You can subscribe
          AFFiNE AI to continue AI experience!
        </div>
        <div class="action-button-group">
          <div @click=${this.config.cancel} class="action-button">Cancel</div>
          <div @click=${this.config.upgrade} class="action-button primary">
            Upgrade
          </div>
        </div>
      `
